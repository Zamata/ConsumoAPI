package com.puyodev.consumoapi
...
import com.puyodev.consumoapi.data.PostApiService
import com.puyodev.consumoapi.data.PostModel

@Composable
fun ScreenPosts(navController: NavHostController, service: PostApiService) {
    var listaPosts: SnapshotStateList<PostModel> = remember { mutableStateListOf() }
    LaunchedEffect(Unit) {
        val listado = service.getUserPosts()
...
@Composable
fun ScreenPost(navController: NavHostController, service: PostApiService, id: Int) {
    var post by remember { mutableStateOf<PostModel?>(null) }
    LaunchedEffect(Unit) {
        val xpost = service.getUserPostById(id)
